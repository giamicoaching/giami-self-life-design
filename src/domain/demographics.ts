...
export const DEMOGRAPHICS_NOTICE =
  '프로그램 이용현황을 파악하기 위해 프로그램 시작·완료·결과 저장 여부와 5년 단위 연령대 및 성별만 개인을 식별하지 않는 통계로 전송됩니다. 정확한 만 나이와 프로그램에서 작성한 삶의 영역 점수, 선택한 가치, 목표, 행동계획 등 응답은 서버로 전송되지 않고 현재 브라우저에만 저장됩니다. 결과화면에서 자발적으로 제출한 도움 정도(1~5점), 가장 도움이 된 과정, 개선 의견만 익명으로 전송됩니다.'
